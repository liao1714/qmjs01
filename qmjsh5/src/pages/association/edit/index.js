...
import { compressImagesOneByOne, convertBase64UrlToFile, changeWxImagesOneByOne } from '@/utils/methons'
...
class Edit extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({ type:'association/associationInfo', payload: {associationPkId: this.props.location.query.pkId} }).then(res => {
      if (res && res.code === 200) {
        let files = []
        res.data.avatarUrl && res.data.avatarUrl.map(item => {
          files.push({
            id: item.id,
  ...
  handleForm =(tagList)=> {
    ...
    let tagName = ''
    tagList.map(item => {
      if (item.value === value.tagPkId[0]) {
        tagName = item.label
      }
    })
    if (
      NotEmpty(this.state.files, '请上传协会头像') &&
      ...
      let body = {
        ...value,
        associationPkId: this.props.location.query.pkId,
        tagPkId: value.tagPkId[0]
      }
      this.setState({
        showQrCode: true,
        associationInfo: {
          pkId: this.props.location.query.pkId,
          associationName: value.associationName,
          features: value.features,
          address: value.address,
          tagName: tagName,
          avatarUrl: [{
            thumbnail: this.state.files[0].url
          }]
        }
      })
      ...
      const that = this
      // body.qrCode = [123]
      // uploadAvatarCropperImages(that.state.files).then(res => {
      //   body.avatarUrl = res
      //   that.props.dispatch({type: 'association/updateAssociation', payload: body}).then(res => {
      //     if (res && res.code === 200) {
      //       Toast.info(res.message, 2)
      //       Router.replace('/association/datum', {associationPkId: that.props.location.query.pkId})
      //     }else {
      ...
      Toast.loading('提交中...', 0)
      setTimeout(function() {
        html2canvas(that.refs.codeImage).then(canvas => {
          let imgData = [{
            file: convertBase64UrlToFile(canvas.toDataURL())
          }]
          uploadOneByOne(imgData).then(res => {
            let item = {
              pkId: that.state.associationInfo.pkId,
              qrCode: res
            }
            that.props.dispatch({type: 'association/addAssociationCode', payload: item}).then(() => {
              body.qrCode = res
              uploadAvatarCropperImages(that.state.files).then(res => {
                body.avatarUrl = res
                that.props.dispatch({type: 'association/updateAssociation', payload: body}).then(res => {
  ...
  render() {
    const { association, index } = this.props
            ...
            <WingBlank size="md" className='container form-container'>
              <List>
                <InputItem
                  ...
                  placeholder='会长姓名'
                  {...getFieldProps('name',{
                    initialValue: association.associationInfo.presidentName,
                  })}
                >会长姓名</InputItem>
                <InputItem
                  readOnly
                  ...
                  {...getFieldProps('mobile',{
                    initialValue: association.associationInfo.mobile,
                  })}
                >会长手机</InputItem>
                <List.Item
                ...
                <InputItem
                  ...
                  {...getFieldProps('associationName',{
                    initialValue: association.associationInfo.associationName,
                  })}
                >协会名称</InputItem>
                <Picker
                  data={tagList}
                  ...
                  {...getFieldProps('tagPkId',{
                    initialValue: [association.associationInfo.tagPkId,],
                  })}
                >
                ...
                <InputItem
                  ...
                  {...getFieldProps('features',{
                    initialValue: association.associationInfo.features,
                  })}
                >协会特色</InputItem>
                <InputItem
                  maxLength={50}
                  ...
                  {...getFieldProps('introduction',{
                    initialValue: association.associationInfo.introduction,
                  })}
                >协会介绍</InputItem>
                <Picker
                  data={city}
                ...
                <InputItem
                  ...
                  {...getFieldProps('address',{
                    initialValue: association.associationInfo.address,
                  })}
                >协会地址</InputItem>
                <List.Item
                  className='require'
                  ...
                  extra={<Switch
                    {...getFieldProps('joinReview', {
                      initialValue: association.associationInfo.joinReview,
